...
from pizza_django.models import Customer, Order, OrderStatus, Pizza, PizzaFlavor, PizzaSize
...
def step_given_orders_from_different_customers(context):
    context.orders = [
        Order.objects.create(customer=Customer.objects.create(
            name='John Doe', address='123 Abc St.')),
        Order.objects.create(customer=Customer.objects.create(
            name='Jane Doe', address='456 Def St.')),
    ]
...
def step_when_filter_orders_by_customer(context, customer_name):
    context.retrieved_orders = list(
        Order.objects.filter(customer__name=customer_name))
...
def step_then_see_all_orders_from_customer(context, customer_name):
    assert all(order.customer.name == customer_name
               for order in context.retrieved_orders)
...
def step_given_placed_order_with_pizzas(context):
    ...
    pizza1 = Pizza.objects.create(order=context.order,
                                  flavor=PizzaFlavor.MARGARITA,
                                  size=PizzaSize.SMALL,
                                  count=1)
    assert pizza1 is not None
    pizza2 = Pizza.objects.create(order=context.order,
                                  flavor=PizzaFlavor.HAWAIIAN,
