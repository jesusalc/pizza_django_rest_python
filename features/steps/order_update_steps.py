...
def step_given_placed_order_with_pizza(context, status):
    ...
    context.order = Order.objects.create(customer=context.customer,
                                         status=status)
    context.pizza = Pizza.objects.create(order=context.order,
...
@when(
    'I update the pizza flavor to "{flavor}" and count to "{count}" and size to "{size}"'
)
...
@when(
    'I try to update the pizza flavor to "{flavor}" and count to "{count}" and size to "{size}"'
)
...
@then(
    'the updated order should have flavor "{flavor}", count "{count}", and size "{size}"'
)
...
def step_impl(context):
    ...
    pizza = Pizza.objects.create(order=order,
                                 flavor='Margherita',
                                 size='Medium',
                                 count=1)
