...
class PizzaOrderTest(TestCase):
    ...
    def test_order_same_flavor_different_sizes(self):
        ...
        margarita_pizzas = self.order.pizzas.filter(flavor=PizzaFlavor.MARGARITA)
        self.assertEqual(margarita_pizzas.count(), 2)
        self.assertEqual(margarita_pizzas.filter(size=PizzaSize.SMALL).count(), 1)
        self.assertEqual(margarita_pizzas.filter(size=PizzaSize.LARGE).count(), 1)
